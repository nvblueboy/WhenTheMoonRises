...
public class ChoiceSelector : MonoBehaviour {
    private static Text txtTopChoice, txtSelectedChoice, txtBotChoice;
    private static GameObject uiDialogue, canChoiceSelector;
    private static List<Choice> choiceList;
    private static bool choicesActive;
    private static float showChoicesTime;
	...
	void Update () {
        ...
        if (choicesActive && select > 0 && Time.time - showChoicesTime > .2f) 
        {
            ...
            DialogueController.Show(selectedChoice.next);
